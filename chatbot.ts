...
function analyzeTopic(
  ...
  const detectedTopic = maxScore > 0
    ? Object.keys(topicScores).find((topic) =>
      topicScores[topic] === maxScore
    ) || "общий"
    : "общий";
...
async function handleMessage(msg: TelegramBot.Message) {
  ...
  // Для групповых чатов реагируем только на упоминания или ответы
  // Для личных чатов: используем весь текст
  let trimmedText: string;
  let shouldRespond = false;

  if (msg.chat.type === "group" || msg.chat.type === "supergroup") {
    const isMention = msg.text.startsWith(`@${botName}`);
    const isReply = msg.reply_to_message?.from?.username === botName;

    if (COMMENT_ALL_MESSAGES || isMention || isReply) {
      trimmedText = isMention
        ? msg.text.replace(`@${botName}`, "").trim()
        : msg.text.trim();
      shouldRespond = true;
      if (isMention) {
      ...
      // Обновляем контекст, но не отвечаем
      shouldAutoComment(chatId, msg.text, userName);
      return;
    }
  } else {
